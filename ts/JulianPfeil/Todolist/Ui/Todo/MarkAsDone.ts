...
export class MarkAsDone {
    ...
    if (buttons) {
      buttons.forEach((button) => {
        button.addEventListener("click", (ev) => {
          ev.preventDefault();
          this.click(button as HTMLElement);
        });
      });
    }
  }
   ...
   * @param data
   */
  async fireRequest(button: HTMLElement, data: MarkAsDoneData) {
    const response = await prepareRequest(button.dataset.endpoint!).post(data).fetchAsResponse();
